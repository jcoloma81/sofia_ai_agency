...
def build_slide_2():
    ...
    draw.text((540, 220), "¿Cansado de perder ventas\nfuera de horario?", fill=(255, 255, 255), font=fonts["title_xl"], anchor="mm", align="center")
    draw.text((540, 350), "Los 3 problemas diarios de distribuidoras y pymes:", fill=(148, 163, 184), font=fonts["subtitle"], anchor="mm")
    ...
    for num, title, desc in problems:
        draw.rounded_rectangle([80, y, 1000, y + 250], radius=28, fill=(24, 18, 26, 220), outline=(239, 68, 68, 160), width=2)
        pw = draw_pill(draw, f"PROBLEMA {num}", 120, y + 35, bg=(239, 68, 68, 40), border=(239, 68, 68, 200), text_color=(252, 165, 165))
        draw.text((120 + pw + 18, y + 36), title, fill=(252, 165, 165), font=fonts["title"])
        draw.text((120, y + 115), desc, fill=(203, 213, 225), font=fonts["body"])
        y += 280

    draw.rounded_rectangle([80, 1330, 1000, 1630], radius=28, fill=(15, 34, 25, 240), outline=(37, 211, 102, 200), width=2)
    draw.text((540, 1400), "💡 La Solución con Sofía:", fill=(37, 211, 102), font=fonts["title"], anchor="mm")
    draw.text((540, 1495), "Atención inmediata 24/7, cuentas exactas\ny los pedidos listos para armar en 1 segundo.", fill=(255, 255, 255), font=fonts["subtitle"], anchor="mm", align="center")
...
def build_slide_3():
    ...
    draw.text((540, 220), "Todo el control desde tu\npropio WhatsApp", fill=(255, 255, 255), font=fonts["title_xl"], anchor="mm", align="center")
    draw.text((540, 350), "Sin contraseñas, sin instalar nada, sin capacitar gente.", fill=(148, 163, 184), font=fonts["subtitle"], anchor="mm")
    ...
    for tag, title, desc in cards:
        draw.rounded_rectangle([80, y, 1000, y + 250], radius=28, fill=(17, 24, 39, 230), outline=(56, 189, 248, 160), width=2)
        pw = draw_pill(draw, tag, 120, y + 35, bg=(56, 189, 248, 40), border=(56, 189, 248, 200), text_color=(56, 189, 248))
        draw.text((120 + pw + 18, y + 36), title, fill=(255, 255, 255), font=fonts["title"])
        draw.text((120, y + 115), desc, fill=(226, 232, 240), font=fonts["body"])
        y += 280

    draw.rounded_rectangle([80, 1330, 1000, 1630], radius=28, fill=(15, 23, 42, 240), outline=(51, 65, 85, 220), width=2)
    draw.text((540, 1400), "📱 100% en la palma de tu mano", fill=(255, 255, 255), font=fonts["title"], anchor="mm")
    draw.text((540, 1495), "El dueño maneja la distribuidora chateando con Sofía\ncomo si fuera su asistente de máxima confianza.", fill=(148, 163, 184), font=fonts["subtitle"], anchor="mm", align="center")
...
def build_slide_4():
    ...
    draw.text((540, 220), "De la consulta al depósito\nen 1 segundo", fill=(255, 255, 255), font=fonts["title_xl"], anchor="mm", align="center")
    draw.text((540, 340), "Mirá el circuito automático paso a paso:", fill=(148, 163, 184), font=fonts["subtitle"], anchor="mm")

    steps = [
        ("PASO 1", "El cliente escribe o manda un audio:", "«Mandame 1 caja de aceite y 2 fardos de harina.»", (255, 255, 255)),
        ("PASO 2", "Sofía calcula con matemática exacta:", "• 1x Aceite Cañuelas: $14.400  |  2x Harina Favorita: $25.000\n💰 TOTAL ESTIMADO: $39.400. ¿Te lo confirmo?", (37, 211, 102)),
    ...
    for tag, step_title, step_desc, accent in steps:
        h = 240 if "\n" in step_desc else 180
        draw.rounded_rectangle([80, y, 1000, y + h], radius=24, fill=(15, 23, 42, 240), outline=(51, 65, 85, 200), width=2)
        pw = draw_pill(draw, tag, 120, y + 25, bg=(30, 41, 59, 220), border=accent, text_color=accent)
        draw.text((120 + pw + 16, y + 26), step_title, fill=(255, 255, 255), font=fonts["body_bold"])
        draw.text((120, y + 80), step_desc, fill=accent if tag == "PASO 2" else (226, 232, 240), font=fonts["body"])
        y += h + 20

    draw.rounded_rectangle([80, 1400, 1000, 1630], radius=26, fill=(20, 33, 25, 240), outline=(37, 211, 102, 200), width=2)
    draw.text((540, 1470), "«Hago el trabajo aburrido", fill=(37, 211, 102), font=fonts["title"], anchor="mm")
    draw.text((540, 1545), "que nadie quiere hacer 😅»", fill=(255, 255, 255), font=fonts["title"], anchor="mm")
...
def build_slide_5():
    ...
    draw.text((540, 220), "Sofía sale a buscar\nclientes todos los días", fill=(255, 255, 255), font=fonts["title_xl"], anchor="mm", align="center")
    draw.text((540, 350), "Tu vendedora digital activa buscando compras:", fill=(148, 163, 184), font=fonts["subtitle"], anchor="mm")
    ...
    y = 430
    for tag, title, desc in cards:
        draw.rounded_rectangle([80, y, 1000, y + 250], radius=28, fill=(17, 24, 39, 230), outline=(250, 204, 21, 150), width=2)
        pw = draw_pill(draw, tag, 120, y + 35, bg=(250, 204, 21, 30), border=(250, 204, 21, 200), text_color=(250, 204, 21))
        draw.text((120 + pw + 18, y + 36), title, fill=(255, 255, 255), font=fonts["title"])
        draw.text((120, y + 115), desc, fill=(226, 232, 240), font=fonts["body"])
        y += 280

    draw.rounded_rectangle([80, 1330, 1000, 1630], radius=28, fill=(15, 23, 42, 240), outline=(37, 211, 102, 200), width=2)
    draw.text((540, 1400), "🛡️ 100% Oficial Meta Cloud API", fill=(37, 211, 102), font=fonts["title"], anchor="mm")
    draw.text((540, 1495), "Sin riesgo de baneo ni emuladores.\nInfraestructura empresarial en la nube activa 24/7.", fill=(255, 255, 255), font=fonts["subtitle"], anchor="mm", align="center")
...
def build_slide_6():
    ...
    draw.rounded_rectangle([80, 1310, 1000, 1600], radius=28, fill=(15, 23, 42, 240), outline=(51, 65, 85, 200), width=2)
    draw.text((540, 1380), "O escribinos directo al WhatsApp:", fill=(148, 163, 184), font=fonts["body_bold"], anchor="mm")
    draw.text((540, 1465), "+54 9 343 572-0312", fill=(56, 189, 248), font=fonts["title"], anchor="mm")
    draw.text((540, 1540), "Sofía AI Agency • Tu socia comercial con IA", fill=(100, 116, 139), font=fonts["caption"], anchor="mm")
